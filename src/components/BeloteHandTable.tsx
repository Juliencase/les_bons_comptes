...
import { cumulativeTeamTotals, handTeamScores, teamName } from '../lib/belote/scoring';
...
export default function BeloteHandTable({ game, onHandPress }: Props) {
  ...
  const columns: ScoreGridColumn[] = game.teams.map((t) => ({ id: t.id, label: teamName(t) }));
  ...
  const rows: ScoreGridRow[] = hands.map((h) => {
    ...
    const takerName = teamName(game.teams.find((t) => t.id === hand.takerTeamId)!);
    return {
      key: h,
      isCurrent,
      values: scores,
      labelExtra: (
        <Text style={{ color: colors.textDim, fontWeight: '400', fontSize: 11 }} numberOfLines={1}>
          {'  '}({takerName}
  ...
  return (
    <ScoreGrid columns={columns} rows={rows} totals={totals} cellWidth={CELL} onRowPress={onHandPress} />
  );
}
